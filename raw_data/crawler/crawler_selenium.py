...
token1155List = pd.read_csv('./token_category.csv')
tokenlist = token1155List.apply(lambda x: tuple(x), axis=1).values.tolist()

hashes = pd.read_csv('./txhash-Noduplicates.csv')
...
for eachtoken in tokenlist:
    ...
    with open("./txs-details/{}.csv".format(eachtoken[0]),"a",newline='', encoding = 'utf-8') as csvfile: 
        ...
        hashes = hashes[hashes['token']==eachtoken[0]]
        ...
        for hash in hashList:
            ...
            url="https://eth.tokenview.io/cn/tx/{}".format(hash)
            # print(url)
            driver.get(url)
            time.sleep(5)
            ...
            #----------------timestamp--------------------- 
            # find timestamp through soup find           
            timestamp = soup.find("TO_FILL").text
            timestamp = timestamp.replace('\\s','')
            timestamp = timestamp.replace('\n','')  

            #-------------------value-----------------------
            # find value through soup find                       
            value = soup.find("TO_FILL").text.replace('\\s','')
            value = value.replace('\n','')
            # print(value)

            #-------------------transactionFee-----------------------
            # find transactionFee through soup find                                   
            transactionFee = soup.find("TO_FILL").text.replace('\\s','')
            transactionFee = transactionFee.replace('\n','')
            # print(transactionFee)

            #-------------------gasPrice-----------------------
            # find gasPrice through soup find 
            gasPrice = soup.find("TO_FILL").text.replace('\\s','')
            gasPrice = gasPrice.replace('\n','')
            # print(gasPrice)

            #------------------find_transactions------------
            # find gasPrice through soup find_all            
            txs = soup.find_all("TO_FILL")
            
            for tx in txs:
                fullStr = str(tx.text).encode(encoding='utf-8')
                fullStr = str(fullStr)
                hrefs = tx.find_all('a', href=re.compile('/cn/token/'))

                #------------------transferedAmount------------
                if '1 ERC-1155 TokenId' in fullStr:
                    transferedAmount = 1
                else:
