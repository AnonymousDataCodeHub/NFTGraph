...
class MyGNNDetector(BaseDetector):
    def __init__(self, train_config, model_config, data):
        super().__init__(train_config, model_config, data)


        model_config['in_feats'] = self.data.graph.ndata['feature'].shape[1]
        model_config['h_feats'] = 64
        model_config['num_classes'] = 1
        model_config['mlp_layers'] = 3

        self.gnn = GraphSAGE(**model_config).to(self.source_graph.device)
        # new_feat = self.gnn(self.source_graph)
        self.gnn1 = GraphSAGE(**model_config).to(self.source_graph.device)
        ...
        # self.mlp = MLP()
        # self.mlp = torch.nn.Sequential(
        #     torch.nn.Linear(model_config['h_feats'] , model_config['h_feats']* 2),
        #     torch.nn.ReLU(),
        #     torch.nn.Linear(model_config['h_feats']* 2, 1),
        #     torch.nn.Sigmoid()).to(train_config['device'])
        # self.mlp1 = torch.nn.Sequential(
        #     torch.nn.Linear(model_config['h_feats'] , model_config['h_feats']*2),
        #     torch.nn.ReLU(),
        #     torch.nn.Linear(model_config['h_feats']*2, ),).to(train_config['device'])
        

        self.mlp = MLP(**model_config).to(self.source_graph.device)

        self.mlp1 = MLP(**model_config).to(self.source_graph.device)

        self.model = torch.nn.Sequential(self.gnn,self.mlp)
        self.model1 = torch.nn.Sequential(self.gnn1,self.mlp1)
        # self.model = GCN(**model_config).to(train_config['device'])

    def train(self):
        train_X = self.data.graph.ndata['feature'][self.train_mask].cpu().numpy()
        train_y = self.data.graph.ndata['label'][self.train_mask].cpu().numpy()
        val_X = self.data.graph.ndata['feature'][self.val_mask].cpu().numpy()
        val_y = self.data.graph.ndata['label'][self.val_mask].cpu().numpy()
        test_X = self.data.graph.ndata['feature'][self.test_mask].cpu().numpy()
        test_y = self.data.graph.ndata['label'][self.test_mask].cpu().numpy()
        weights = np.where(train_y == 0, 1, self.weight)
        ...
        for e in range(self.train_config['epochs']):
            ...
            h = self.gnn(self.data.graph.to(self.train_config['device']))
            logits = h.reshape(-1)
            # logits = self.mlp(h)
            h1 = self.gnn1(self.data.graph.to(self.train_config['device']))
            # logits1 = self.mlp1(h1)
            logits1 = h1.reshape(-1)
            loss_ces = torch.nn.CrossEntropyLoss()(logits[self.train_mask], self.labels[self.train_mask].float())
        
        #Pseudo label
            p = torch.tensor([0.2,0.8])
            p1,p9 = torch.quantile(logits[self.train_mask],p.to(self.train_config['device']))
            # print(p1,p9)
            logits_unobserved = logits[~self.train_mask]
            tar1 = logits_unobserved[torch.where(logits_unobserved < p1)].reshape(-1, 1)
            tar2 = logits_unobserved[torch.where(logits_unobserved > p9)].reshape(-1, 1)

            tar1 = torch.clamp(tar1, 0.005, 1) * torch.log(torch.clamp(tar1, 0.005, 1))
            tar2 = torch.clamp(tar2, 0.005, 1) * torch.log(torch.clamp(tar2, 0.005, 1))

            tar1 = -torch.mean(torch.where(torch.isnan(tar1), torch.zeros_like(tar1), tar1))
            tar2 = -torch.mean(torch.where(torch.isnan(tar2), torch.zeros_like(tar2), tar2))

            pseudo_loss = tar1 + tar2
            lambda1 = 1
            lambda2 = 10
            
            loss = lambda1*loss_ces + lambda2*pseudo_loss
            # print(loss_ces,pseudo_loss,loss)
            ...
            if self.model_config['drop_rate'] > 0:
                ...
                logits = self.model(self.source_graph)
            probs = logits
            val_score = self.eval(self.labels[self.val_mask], probs[self.val_mask])
            if val_score[self.train_config['metric']] > self.best_score:
                self.patience_knt = 0
                self.best_score = val_score[self.train_config['metric']]
                test_score = self.eval(self.labels[self.test_mask], probs[self.test_mask])
            else:
                self.patience_knt += 1
                if self.patience_knt > self.train_config['patience']:
                    break
