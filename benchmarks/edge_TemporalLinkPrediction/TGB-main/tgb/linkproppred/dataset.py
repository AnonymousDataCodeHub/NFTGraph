...
class LinkPropPredDataset(object):
    ...
    def generate_processed_files(self) -> pd.DataFrame:
        ...
        if (osp.exists(OUT_DF)) and (self.version_passed is True):
            ...
            elif self.name == "tgbl-tinynft":
                df, edge_feat, node_ids = load_edgelist_tinynft(self.meta_dict["fname"])
...
def main():
    ...
    # meta_dict = {}
    # meta_dict['dir_name'] = 'tgbl_tinynft'
    # meta_dict["nodefile"] = None
    # meta_dict['fname'] = '/data/sx/TGB-main/tgb/datasets/tgbl_tinynft/tgbl-tinynft_edgelist.csv'
    data = PyGLinkPropPredDataset(name=name, root="/data/sx/TGB-main/tgb/datasets")
    data = data.get_TemporalData()
    ...
    def getTemporalData(x):
        return TemporalData(src=x.src[train_mask], dst=x.dst[train_mask], t=x.t[train_mask],msg=x.msg[train_mask],y=x.y[train_mask])


    train_data = getTemporalData(data)


    negs = NegativeEdgeGenerator(dataset_name=name,first_dst_id=0,last_dst_id=22109,num_neg_e=20,strategy='hist_rnd',historical_data=train_data)

    negs.generate_negative_samples(data,split_mode='val',partial_path='/data/sx/TGB-main/tgb/datasets/tgbl_tinynft')
    negs.generate_negative_samples(data,split_mode='test',partial_path='/data/sx/TGB-main/tgb/datasets/tgbl_tinynft')
