...
parser.add_argument('--dataset', type=str, default='ogbl-tinynftgraph')
...
args.res_dir = os.path.join(f'./results{"_NoTest" if args.no_test else ""}',f'{args.dataset.split("-")[1]}-{args.model}+{time.strftime("%m%d%H%M%S")}',)
