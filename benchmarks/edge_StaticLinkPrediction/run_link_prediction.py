...
parser.add_argument('--trials', type=int, default=10)  # Number of random search
...
for model_name in models:
    ...
    train_config = {
        'device': device,
        'epochs': min(model_config['epoch'],args.epochs),  #traing epoch for models
    ...
    final_val_scores, test_scores = [],[]
    train_config['batch_size'] = args.batchsize
    print_log(train_config)
    ...
    for t in range(args.trials):
        ...
        final_val_scores.append(best_finalval)
        ...
        del model
    del train_graph
    model_result[dataset_name+'-finalval_score mean'] = np.mean(final_val_scores)
    model_result[dataset_name+'-finalval_score std'] = np.std(final_val_scores)
    model_result[dataset_name+'-test_score mean'] = np.mean(test_scores)
    model_result[dataset_name+'-test_score std'] = np.std(test_scores)
